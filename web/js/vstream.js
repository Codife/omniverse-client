...
class VStream {
  ...
  construct_ui() {
    ...
    this.vframe.addEventListener("mousemove", function (event) {
      that.mouseMoveHandler(event);
    });
    this.vframe.addEventListener("mouseout", function (event) {
      that.mouseOutHandler(event);
    });
    this.vframe.addEventListener("mousedown", function (event) {
      that.mouseDownHandler(event);
    });
    this.vframe.addEventListener("contextmenu", function (event) {
      that.contextMenuHandler(event);
    });
    this.vframe.addEventListener("mouseup", function (event) {
      that.mouseUpHandler(event);
    });
    this.vframe.addEventListener("mousewheel", function (event) {
      that.mouseWheelHandler(event);
    });
    this.vframe.addEventListener("keydown", function (event) {
      that.keyDownHandler(event);
    });
    this.vframe.addEventListener("keyup", function (event) {
      that.keyUpHandler(event);
    });
    this.vframe.addEventListener("keypress", function (event) {
      that.keyPressHandler(event);
    });
    window.addEventListener("resize", function (event) {
      that.resizeHandler(event);
    });
  ...
  connectToServer() {
    if ("WebSocket" in window) {
      ...
      try {
        ...
        var that = this;
        this.videosocket.onopen = function (result) {
          that.onVideoConnectedCallback(result);
        };
        this.videosocket.onmessage = function (indata) {
          that.decodeAndDisplayVideo(indata);
        };
        this.videosocket.onerror = function (obj) {
          that.onerror(obj);
        };
        this.videosocket.onclose = function (obj) {
          that.onVideoClose(obj);
        };
      } catch (exception) {
  ...
  decodeAndDisplayVideo(indata) {
    ...
    if (typeof indata.data === "string") console.log(indata.data);
  ...
  mouseMoveHandler(event) {
    ...
    if (this.video.connected) {
      // && this.mouseDown.some( function(val){ return val } ))
  ...
  resizeHandler() {
    ...
    if (this.videosocket) this.videosocket.send(JSON.stringify(command));
